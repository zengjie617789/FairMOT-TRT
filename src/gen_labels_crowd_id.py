...
def odgt_to_txt(file_path):
    res = load_func(file_path)
    save_path = file_path.replace('odgt', 'txt')
    with open(save_path, 'a') as f:
        print(res, file=f)


if __name__ == '__main__':
    root_path = os.path.expanduser("~/zzzj/projects/")
    print(root_path)
    data_val = root_path + '/crowdhuman/images/val'
    label_val = root_path + '/crowdhuman/labels_with_ids/val'
    ann_val = root_path + '/crowdhuman/annotation_val.odgt'
    data_train = root_path + '/crowdhuman/images/train'
    label_train = root_path + '/crowdhuman/labels_with_ids/train'
    ann_train = root_path + '/crowdhuman/annotation_train.odgt'
    gen_labels_crowd(data_train, label_train, ann_train)
    gen_labels_crowd(data_val, label_val, ann_val)
    # odgt_to_txt(ann_train)
