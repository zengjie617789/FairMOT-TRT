...
class LoadVideo:  # for inference
    def __init__(self, path, img_size=(1088, 608)):
        ...
        # self.w, self.h = 640, 480  #for MOT16-05
    ...
    def __next__(self):
        ...
        img_path = os.path.join(os.getcwd(), 'temp_pic')
        # if not os.path.exists(img_path):
            # os.makedirs(img_path)
        # img_path = os.path.join(img_path, str(self.count)+'_letterbox.jpg')
        # cv2.imwrite(img_path , 255 * img.transpose((1, 2, 0))[:, :, ::-1])  # save letterbox image
...
class JointDataset(LoadImagesAndLabels):  # for training
    ...
    def __init__(self, opt, root, paths, img_size=(1088, 608), augment=False, transforms=None):
        ...
        for ds, path in paths.items():
            with open(path, 'r') as file:
                ...
                self.img_files[ds] = [osp.join(root, x.strip()) for x in self.img_files[ds]] #['crowdhuman/images/train/284193,faa9000f2678b5e.jpg']
    ...
    def __getitem__(self, files_index):
        ...
        ret = {'input': imgs, 'hm': hm, 'reg_mask': reg_mask, 'ind': ind, 'wh': wh, 'reg': reg, 'ids': ids, 'bbox': bbox_xys, 'img_path':img_path}
        return ret
