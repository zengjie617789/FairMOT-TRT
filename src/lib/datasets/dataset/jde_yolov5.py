...
class LoadImageSTN():
        def __init__(self, path, img_size=(1088, 608)):
            if os.path.isdir(path):
                ...
                self.files = sorted(glob.glob('%s/*.*' % path))
                self.files = list(filter(lambda x: os.path.splitext(x)[1].lower() in image_format, self.files))
                self.labels = [ x.replace('images', 'labels_with_id').replace('.png', '.txt').replace('.jpg', '.txt') for x in self.files]
            elif os.path.isfile(path):
                self.files = [path]
...
class LoadVideo:  # for inference
    def __init__(self, path, img_size=(1088, 608)):
        ...
        self.w, self.h = 1920, 1080   # the size is evaluated for MOT16, the saved img size
...
class LoadImagesAndLabels:  # for training
    ...
    def __getitem__(self, img_files, label_files, index):
        ...
        # augment_hsv(img, hgain=0.4, sgain=0.7, vgain=0.015)
        nL = len(labels)
        if nL > 0:
    ...
    def get_data(self, img_files, label_files, index):
        use_mosaic = True if random.random() < 0.5 else False
        if use_mosaic:
            img, labels = self.load_mosaic_ori(img_files, label_files, index)
        else:
            img, labels = self.load_mosaic(img_files, label_files, index)
        # MixUp
        # if random.random() < 0.0: # epoch > max_epoch/2
        if random.random() < 0.5:
        ...
        # augment_hsv(img, hgain=0.015, sgain=0.7, vgain=0.4) #original
        augment_hsv(img, hgain=0.4, sgain=0.7, vgain=0.015)
        nL = len(labels)
        if nL > 0:
            # convert xyxy to xywh
...
class JointDataset(LoadImagesAndLabels):  # for training
    ...
    def __init__(self, opt, root, paths, img_size=(1088, 608), augment=False, transforms=None):
        ...
        self.nds = [len(x) for x in self.img_files.values()] # number datasets
        self.cds = [sum(self.nds[:i]) for i in range(len(self.nds))]# current dataset start
        self.nF = sum(self.nds) #number Files
        self.width = img_size[0]
        self.height = img_size[1]
        self.max_objs = opt.K
        self.augment = augment
