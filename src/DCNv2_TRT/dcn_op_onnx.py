...
import _ext_op_v2 as _backend
from torch.onnx import register_custom_op_symbolic

class DeformableConv2DFunction(Function):

    @staticmethod
    def symbolic(g, input, weight, bias, offset, mask, stride, padding, dilation, deformable_groups):
        return g.op("DCNv2_TRT", input, weight, bias, offset, mask, name_s="DCNv2_TRT",
                    info_s=json.dumps({
                        "dilation": dilation,
                        "padding": padding,
                        "stride": stride,
    ...
    def backward(ctx, *grad_outputs):
        ...
        grad_input, grad_weight, grad_bias, grad_offset, grad_mask = _backend.backward(
            input_tensor,
            weight,
            bias,
            offset,
...
class DeformableConv2DLayer(Module):
    ...
    def forward(self, inputs):
        ...
        return DeformableConv2DFunction.apply(inputs, self.weight, self.bias, offset, mask, self.stride, self.pad,
...
if __name__ == "__main__":
    ...
    def check_gradient_dconv():
        ...
        gradcheck(func, (t, weight, bias, offset, mask, (1, 1), (1, 1), (1, 1), deformable_groups), eps=1e-3, atol=1e-3,
                  rtol=1e-2)
        # res = func(t, weight, bias, offset, mask, (1,1), (1,1), (1,1), deformable_groups)
        # print(res)

        # gradcheck(func, (t, weight.type(torch.float64), bias.type(torch.float64), offset.type(torch.float64), mask.type(torch.float64), (1,1), (1,1), (1,1), deformable_groups), eps=1e-3, atol=1e-3, rtol=1e-2)
